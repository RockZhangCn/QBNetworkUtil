...
public class NetBasicInfo
{
    ...
    /*
    private Thread mDnsResolverThread = new Thread(new Runnable()
    {
        public void run()
        {
            ...
            try
            {
                InetAddress aaa = InetAddress.getByName("3gimg.qq.com");
                InetAddress[] addrs = InetAddress.getAllByName("3gimg.qq.com");
                sb.append("Begin: \n" + aaa.toString() + "\nEnd\n");
                for (InetAddress adr : addrs)
                {
                    sb.append(adr.toString() + "\n");
                    Log.e("ROCK", "Domain addr " + adr.toString());
                }

                Log.e("ROCK", "Domain query finished");
                //m_handler.sendEmptyMessage(CMD1);

							if(NetInfoUtils.postStrContent("123456789ROCKZHANG"))
							{
								m_handler.sendEmptyMessage(CMD1);
							}
				            else
				            {
				            	m_handler.sendEmptyMessage(CMD2);
				            }
            ...
            catch (Exception e)
            {
                ...
                e.printStackTrace();
                Message exceptMsg = m_handler.obtainMessage();
                exceptMsg.obj = e.toString();
                exceptMsg.what = CMD3;
                m_handler.sendMessage(exceptMsg);
            }

            Message exceptMsg = m_handler.obtainMessage();
            exceptMsg.obj = sb.toString();
            exceptMsg.what = CMD2;
            m_handler.sendMessage(exceptMsg);
